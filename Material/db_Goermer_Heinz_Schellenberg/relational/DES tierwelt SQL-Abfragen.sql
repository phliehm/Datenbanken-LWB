...
SELECT AD_ID, Faelligkeit, Bezeichnung, Beschreibung FROM Arbeitsdiensttypen NATURAL JOIN (SELECT * FROM Arbeitsdienste WHERE DATE_PART('week',Faelligkeit) = DATE_PART('week', CURRENT_DATE)) AS xyz ORDER BY Faelligkeit;
